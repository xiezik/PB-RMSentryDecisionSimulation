...
ErrorInfo JudgesysRobot::Init(std::string robot_num, std::string color){
    ...
    judgeStatus_pub_ = nh_.advertise<rm_interfaces::RobotStatus>("judgeSysInfo/" + robot_num + "/status",5);
    judgeHeat_pub_ = nh_.advertise<rm_interfaces::RobotHeat>("judgeSysInfo/" + robot_num + "/heat_power",5);
...
void JudgesysRobot::RawVelCallback(const geometry_msgs::Twist::ConstPtr& msg)
{
  ...
  if(raw_cmd_selector_count<50)
	...
	if (!begin_) {
    begin_ = true;
        ROS_INFO("[rm_judgesys]%s robot:%s raw cmd vel received!",color_.c_str(),robot_num_.c_str());
  }
	new_cmd_acc_ = true;
	raw_cmd_vel_ = *msg;
}
...
void JudgesysRobot::RawVelCallback_act(const geometry_msgs::Twist::ConstPtr& msg)
{
  if (!begin_) {
    begin_ = true;
        ROS_INFO("[rm_judgesys]%s robot:%s raw cmd vel(act) received!",color_.c_str(),robot_num_.c_str());
  }
  new_cmd_acc_ = true;
  raw_cmd_vel_ = *msg;
...
void JudgesysRobot::BuffReload()
{
    ...
    ROS_INFO("[rm_judgesys]%s %s Buff Reload! ammo + 200.",color_.c_str(),robot_num_.c_str());
}
...
void JudgesysRobot::BuffHeal()
{
  if(!IsAlive())
    ...
    ROS_INFO("[rm_judgesys]%s %s Buff Heal! Hp + 200.",color_.c_str(),robot_num_.c_str());
}
...
void JudgesysRobot::Shoot(int num, float speed)
{
    ...
    if(ammo_ > 0)
       ammo_ -- ;
    if(ammo_ <= 0)
        ROS_ERROR("[rm_judgesys] ammo is less than zero!");
}
...
void JudgesysRobot::DebuffMove()
{
    ...
    ROS_INFO("[rm_judgesys]%s %s Debuff: Can't move!",color_.c_str(),robot_num_.c_str());
}

void JudgesysRobot::DebuffShoot()
{
    shoot_debuff_time_ = 10;
    ROS_INFO("[rm_judgesys]%s %s Debuff: Can't shoot!",color_.c_str(),robot_num_.c_str());
}
...
void JudgesysRobot::BeHit(rm_common::JudgeSysCommand command)
{
    ...
    case JudgeSysCommand::ARMOR_HIT_RIGHT:
        ...
        break;
    default:
        ROS_ERROR("[rm_judgesys]Wrong armor hit command!");
        break;
    }
    if(health_<0)
        health_ = 0;
