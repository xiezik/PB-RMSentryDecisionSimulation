...
using rm_judgesys::ErrorCode;
using rm_common::ErrorInfo;
using rm_common::JudgeSysCommand;
using namespace std;
...
void JudgesysControl::SetGamePhase(int phase)
{
    if(phase == JudgeSysCommand::GAME_PERP)
    {
        ...
        game_time = 180;
        ROS_INFO("[rm_judgesys]Set game preperation");
    }
    else if(phase == JudgeSysCommand::GAME_START)
    {
        gamePhase_ = GamePhase::START;
        game_time = 180;
        ROS_INFO("[rm_judgesys]Set game start");
        //RFID_Refresh();
        ResetAllRobot();
    }
    else if(phase == JudgeSysCommand::GAME_END)
    {
        ...
        game_time = 0;
        ROS_INFO("[rm_judgesys]Set game end");
    }
...
ErrorInfo JudgesysControl::Init() {
    ...
    gameState_pub_ = nh_.advertise<rm_interfaces::GameStatus>("judgeSysInfo/game_state", 5);
    buffInfo_pub_ = nh_.advertise<rm_interfaces::Buffinfo>("judgeSysInfo/buff_info", 5);
    shoot_hit_sub_ = nh_.subscribe<rm_interfaces::JudgeSysShootHit>("judgeSysInfo/shoot_hit_event",100,&JudgesysControl::ShootHitCallback,this);
    for(int i=0;i<6;i++)
        buffZone[i] = false;
    game_time = 0;
    gamePhase_ = GamePhase::PREPERATION;
...
void JudgesysControl::ShootHitCallback(const rm_interfaces::JudgeSysShootHit::ConstPtr &msg)
{
...
bool JudgesysControl::handle_function(rm_interfaces::JudgeSysControl::Request &req,
...
void JudgesysControl::UpdateInfo()
{
    ...
    rm_interfaces::GameStatus gameState;
    ...
    rm_interfaces::Buffinfo buffInfo;
    for(int i=0;i<6;i++)
    {
...
bool JudgesysControl::ReloadRobot(std::string robot_num)
{
    bool val = false;
     for (auto it = robots_.begin(); it != robots_.end(); ++it) {
        ...
        {
            (*it)->Reload();
                        ROS_INFO("[rm_judgesys]Reload %s ammo = %d\n",(*it)->GetName().c_str(),(*it)->GetAmmo());
                        val = true;
        }
    }
    return val;
...
void JudgesysControl::RFID_Refresh()
{
    ...
    ROS_INFO("[rm_judgesys]RFID refresh!");
    for(int i =0;i<6;i++)
    {
        RFID[i].isActivated = false;
        ROS_INFO("[rm_judgesys]RFID F%d:%s %s",i+1, RFID[i].color.c_str(),type[RFID[i].type - RFID_type::HEAL].c_str());
    }
}
...
void JudgesysControl::LogOutput()
{
    ...
    for(int i = 0;i < 4;i++)
    {
        if(robot_health[i] > 0 && robots_[i]->GetHealth() <=0)
            ROS_INFO("[rm_judgesys]%s is dead!",robots_[i]->GetName().c_str());
...
int main(int argc, char *argv[])
{
    ...
    rm_judgesys::JudgesysControl judgesys_control;
    int ms = 0;
    int ms_last = 0;
    struct timeval tv;
    struct timezone tz;
